...
--Sprite 'data'
--The first one was mostly just to test/for clean up if I ever felt like contributing
...
function SetSelfSprite()
    local SpriteNo = PlayerExtra3[PlayerID]
    local PalIndexAddress = (0x3003128) + 0x5
    local PalAddress = 0x20377f8
    local SpriteArray = {}
    local PalArray = {}
    if SpriteNo < 3 then
        PalArray = FRLGPlayerPal
    elseif SpriteNo == 3 then
    --item ball
        SpriteArray = ItemBallSprite
        PalArray = ItemBallPal
    end
    
    local SpriteTempVar0 = 0x6010060
    for i = 1, #SpriteArray do
        emu:write32(SpriteTempVar0, SpriteArray[i])
        SpriteTempVar0 = SpriteTempVar0 + 4
    end
    temp = emu:read8(PalIndexAddress)
    temp = temp & 0x0F
    --we reset it to palette index 0
    emu:write8(PalIndexAddress, temp)
    
    SpriteTempVar0 = PalAddress
    for i = 1, #PalArray do
        emu:write16(SpriteTempVar0, PalArray[i])
...
function HandleSprites()
	...
	for i = 1, MaxPlayers do
		PlayerChar = i - 1
        if (PlayerID == i) then
            createChars(PlayerChar,0,PlayerExtra3[i])
        end
		if PlayerID ~= i and PlayerIDNick[i] ~= "None" then
            --Hiding
...
function ReceiveData()
	...
	if EnableScript == true then
			...
			if SocketMain:hasdata() then
				local ReadData = SocketMain:receive(68)
